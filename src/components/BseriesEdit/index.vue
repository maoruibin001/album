<template>
    <modal :visible.sync="editDiologShow" :styleObj="styleObj" @close="editDiologShow=false" :title="(isEdit ?'编辑' : '新增') + '大系列'">
        <template v-slot:body>
                    <div class="form-container">
                      <div class="form">
                        <div class="item name">
                          <div class="left">大类名称</div>
                          <div class="right"><input type="text" placeholder="请输入大系列名称" v-model="editInfo.name"></div>
                        </div>
...
export default {
  ...
  computed: {
    editDiologShow: {
      get () {
        return store.state.bEditDiologShow
      },
      set (val) {
        store.commit('setBEditDiologShow', val)
      }
    },
    isEdit () {
      return store.state.isBEdit
    },
    editInfo: {
      get () {
        return store.state.bEditInfo
      },
      set (val) {
        store.commit('setBEditInfo', val)
      }
    }
  },
  methods: {
    ...
    save () {
      ...
      store.dispatch(this.isEdit ? 'editBseries' : 'addBseries', this.editInfo).then(() => {
